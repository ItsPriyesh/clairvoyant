...
class Packet:
	...
	def from_dict(data):
		...
		builder.set_type(data['type'])
		builder.set_node_id(data['node_id'])
		builder.set_message_id(data['message_id'])
		builder.set_ttl(data['ttl'])
		builder.set_retry_count(data['retry_count'])
		builder.set_hop_count(data['hop_count'])

		if (data['type'] == 'ML_CLASS'):
			builder.set_payload(MlPayload.from_dict(data))
		elif (data['type'] == 'HEART_BEAT'):
			builder.set_payload(HeartbeatPayload.from_dict(data))
		elif (data['type'] == 'MOTION_EVENT'):
			builder.set_payload(MotionPayload.from_dict(data))
		elif (data['type'] == 'ACK'):
			builder.set_payload(AckPayload.from_dict(data))
...
class MlPayload:
	...
	def from_dict(data):
		payload = MlPayload()
		payload._battery_lvl = data['battery_level']
		payload._timestamp = data['timestamp']
		payload._classification = data['classification']
		payload._confidence = data['confidence']
...
class MotionPayload:
	...
	def from_dict(motion):
		payload = MotionPayload()
		payload._battery_lvl = motion['battery_level']
		payload._timestamp = motion['timestamp']
		payload._motion_type = motion['motion_type']
		payload._orientation = motion['orientation']
		payload._roll = motion['roll']
		payload._pitch = motion['pitch']
		payload._yaw = motion['yaw']
...
class HeartbeatPayload:
	...
	def from_dict(heartbeat):
		payload = HeartbeatPayload()
		payload._battery_lvl = heartbeat['battery_level']
		payload._timestamp = heartbeat['timestamp']
		return payload
