...
@WebSocket
public class MotionEventSocketHandler {

    private static final FluentLogger logger = FluentLogger.forEnclosingClass();

    private final Gson gson;
    private final UserStore userStore;
    private final MotionEventPublisher motionEventManager;
    private final Map<Session, Disposable> clients = new ConcurrentHashMap<>();

    @Inject
    public MotionEventSocketHandler(MotionEventPublisher motionEventManager, UserStore userStore, Gson gson) {
        this.motionEventManager = motionEventManager;
    ...
    private void startListening(Session session, int userID) {
        ...
        Disposable disposable = motionEventManager.listenForUser(userID)
                .map(proto -> MotionEventAuto.builder()
                        .setMotionEventId(proto.getMessageId())
                        .setNodeId(proto.getNodeId())
                        .setCreatedAt(new Timestamp(proto.getTimestamp() * 1000L))
                        .setMotionType(proto.getMotionType())
                        .setOrientation(proto.getOrientation())
                        .setPitch(proto.getPitch())
                        .setRoll(proto.getRoll())
                        .setYaw(proto.getYaw())
                        .build()
                )
                .subscribeOn(Schedulers.newThread())
